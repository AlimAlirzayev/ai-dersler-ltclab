"""Single source of truth for the lesson-36 homework notebook.

Writes two artefacts from the same cell list:
  * ders36_ev_tapsirigi.ipynb  - upload/backup copy
  * hucreler.md               - copy-paste view for the already-open Colab
...
# (kind, title, source) -- kind is "md" or "code"
CELLS = []

CELLS.append(("md", "Basliq", """# Dərs #36 — Ev tapşırığı: modelə `y = 5x` öyrətmək
...
CELLS.append(("code", "Hazirliq yoxlamasi", r"""# ----- 0. HAZIRLIQ (dərsin 2-ci bölməsi) -----
import torch
...
CELLS.append(("code", "Esas hell", r"""import torch
...
CELLS.append(("code", "7. YOXLAMA", r"""# ----- 7. YOXLAMA -----
...
CELLS.append(("md", "Durust oxunus", """## Nəticənin dürüst oxunuşu
...
CELLS.append(("code", "Subut ve davam", r"""# Qalıq b — 47.3 rəqəminin bütün izahı buradadır.
...
CELLS.append(("code", "Muqayise qrafiki", r"""# ----- ƏLAVƏ QRAFİK: 200 epoch və 2000 epoch yanaşı -----
SURFACE = "#fcfcfb"
...
def build_ipynb(path):
    ...
    nb = {
        "cells": cells,
        "metadata": {
            "colab": {"provenance": [], "name": "Ders36_ev_tapsirigi_y5x.ipynb"},
...
def build_md(path):
    ...
    for i, (kind, title, src) in enumerate(CELLS, 1):
        etiket = "MƏTN (Text)" if kind == "md" else "KOD (Code)"
        out.append(f"## Hüceyrə {i} — {etiket} — {title}")
        out.append("")
        out.append("````markdown" if kind == "md" else "```python")
        out.append(src.rstrip("\n"))
        out.append("````" if kind == "md" else "```")
        out.append("")
    path.write_text("\n".join(out), encoding="utf-8")


if __name__ == "__main__":
    build_ipynb(HERE / "ders36_ev_tapsirigi.ipynb")
    build_md(HERE / "hucreler.md")
    print("wrote ders36_ev_tapsirigi.ipynb and hucreler.md")
